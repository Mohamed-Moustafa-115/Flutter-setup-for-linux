...
USER_NAME=$(whoami)
cd /home/$USER_NAME
echo "\e[46mCreating a directory for Flutter requirements\e[0m"
mkdir "Flutter Requirements"

#Install required packages
echo "\e[46mUpdating and installing required packages\e[0m"
sudo apt-get update -y && sudo apt-get upgrade -y
sudo apt-get install -y curl unzip xz-utils zip libglu1-mesa

#Install FlutterSDK
echo "\e[46mInstalling FlutterSDK\e[0m"
...
echo "\e[46mDownloading JDK 17.0.12\e[0m"
curl -# -O https://download.oracle.com/java/17/archive/jdk-17.0.12_linux-x64_bin.deb
ls -l
echo "\e[46mInstalling JDK 17.0.12 (Enter your sudo password)\e[0m"
sudo apt install ./jdk-17.0.12_linux-x64_bin.deb -y

#Download VS Code
echo "\e[46mDownloading VS Code\e[0m"
...
echo "\e[46mInstalling VS Code\e[0m"
sudo apt install ./code_1.124.2-1781225536_amd64.deb -y

#Download and install Android studio
echo "\e[46mDownloading Android studio\e[0m"
...
while true; do
  ...
  if [ -n "$GITHUB_ACTIONS" ]; then
    # In GitHub Actions, skip prompt or use env var
      ...
      read -p "\e[46mDo you want to keep Flutter Requirements folder (Setup packages) y/n\e[0m" CHOICE
  fi
  if [ "$CHOICE" = "n" ] || [ "$CHOICE" = "N" ]; then
    sudo rm -rf "Flutter Requirements"
    echo "\e[46mFlutter Requirements folder is removed\e[0m"
    echo "\e[46mSetup finsihed, now run Andriod Studio to download the remaining SDK components\e[0m"
    break
  elif [ "$CHOICE" = "y" ] || [ "$CHOICE" = "Y" ]; then
    echo "\e[46mSetup finsihed, now run Android Studio to download the remaining SDK components\e[0m"
    break
  else
  echo "\e[41mInvalid input\e[0m"
  fi
done
